...
```
각 사다리의 수는 피보나치 수와 같고 2^30승으로 먼저 구해주고 2^N 은 2^30의 배수이기 때문에 영향을 받지 않는다고 생각했다. 각 dp의 수와 나머지 연산으로 값을 구해주면 된다.
```

### MinMaxDivision

```
이분탐색을 통해 특정 합이 만족했을 때 사다리의 개수가 적으면 수가 너무 큰것이라 hi를 땡기고 사다리의 수가 많으면 수가 너무 작은 것이라 생각하고 해결하면 된다.
```

### NailingPlanks

```
널빤지와 0 ~ mid 까지 못을 정렬을 시켜주는게 100점 솔루션이였다. 이분탐색은 이해하기 쉬우나 시간복잡도에 대한 생각을 더 해야한다. 못을 정렬시키지 않으면, 앞의 널빤지를 모두 봐야해서 시간초과가 나는 것 같다.
```

### AbsDistinct

```
수의 범위가 커서 배열을 체크하는 것으로 해결이 불가능했고, set을 이용하여 log n 시간에 찾을 수 있게했다.
```

### CountDistinctSlices

```
원소가 추가될 때마다, 부분집합의 수가 늘어나는데 그 수는 원소의 개수만큼 늘어나게 된다. 거기에 자기자신의 유일한 부분집합까지 더하는 연산은
cnt += j-i + 1이다.
i를 당기는 행위는 부분집합의 수가 늘어나는 것이 아니므로 그냥 넘어가도 된다.
```

### CountTriangles

```
원소 정렬후 두 원소의 합보다 큰 lower_bound를 구하고 개수를 더해주면 n^2 log n 으로 답을 구할 수 있다.
```

### MinAbsSumOfTwo

```
중복을 피하는 방법으로 정렬후 양 끝에서 절대값이 큰쪽을 당겨오면서 가장 작은 합을 구하면된다. 그렇게 하나가 남은 상태에서 그 수의 2배의 절대값과 오면서 구한 가장 작은 합 중에 더 작은것이 정답이다. 설명할 자신이 없고 이해하지 못했다.
```

### MaxNonoverlappingSegments

```
익숙한 그리디 문제 회의실 문제와 비슷하게 끝나는 지점으로 정렬하면 가장 많은 회의실을 빌릴 수 있다.
```

### TieRopes

```
처음부터 k 이상의 길이가 만들어지면 ++ 하는 방식으로 간단한 그리디 문제
```
